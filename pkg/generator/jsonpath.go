...
// In string s, replace the substr old, at index i, with substr newStr.
func replace(s string, i int, old, newStr string) string {
	if i+len(old) > len(s) {
		return fmt.Sprintf("%s%s", s[0:i], newStr)
	}
	return strings.Join([]string{s[0:i], s[i+len(old):]}, newStr)
}
