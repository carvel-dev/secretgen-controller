...
ytt -f config/ -f config-build -f config-deploy -f config-release -v secretgen_controller_version="$(get_sgctrl_ver)" | kbld --imgpkg-lock-output config/.imgpkg/images.yml -f- > ./tmp/release.yml
