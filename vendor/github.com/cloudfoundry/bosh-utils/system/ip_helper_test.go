...
var _ = Describe("CalculateNetworkAndBroadcast", func() {
	...
	Context("ipv4", func() {
		It("calculates network and broadcast", func() {
			network, broadcast, maskSize, err := CalculateNetworkAndBroadcast("192.168.195.6", "255.255.255.0")
			Expect(err).ToNot(HaveOccurred())
			Expect(network).To(Equal("192.168.195.0"))
			Expect(broadcast).To(Equal("192.168.195.255"))
			Expect(maskSize).To(Equal(24))
		})
	})

	Context("ipv6", func() {
		It("returns an empty network and broadcast", func() {
			network, broadcast, maskSize, err := CalculateNetworkAndBroadcast("fd6b:6e04:558d:ebe::1", "ffff:ffff:ffff:ffff::")
			Expect(err).ToNot(HaveOccurred())
			Expect(network).To(Equal(""))
			Expect(broadcast).To(Equal(""))
			Expect(maskSize).To(Equal(0))
		})
	})
})
