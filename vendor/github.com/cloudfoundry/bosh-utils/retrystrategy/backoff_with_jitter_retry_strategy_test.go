...
var _ = Describe("BackoffWithJitterRetryStrategy", func() {
	...
	Describe("Try", func() {
		It("includes type of retryable in log message", func() {
			...
			attemptRetryStrategy := NewBackoffWithJitterRetryStrategy(3, 0, 0, retryable, logger)
		...
		Context("when the attempt is not retryable", func() {
			It("stops trying", func() {
				...
				attemptRetryStrategy := NewBackoffWithJitterRetryStrategy(10, 0, 0, retryable, logger)
				err := attemptRetryStrategy.Try()
				Expect(err).To(HaveOccurred())
